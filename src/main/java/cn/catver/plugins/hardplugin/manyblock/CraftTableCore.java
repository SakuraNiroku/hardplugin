...
public class CraftTableCore {

    public static void playerhitblock(PlayerInteractEvent e){
        if(e.hasBlock() && e.getAction() == Action.LEFT_CLICK_BLOCK){
            playerhitblockfunc_coal(e);
            playerhitblockfunc_rawcopper(e);
        }
    }

    private static void playerhitblockfunc_coal(PlayerInteractEvent event){
        if(event.hasBlock()){
    ...
    private static void playerhitblockfunc_rawcopper(PlayerInteractEvent event){
        if(event.hasBlock()){
            if(event.getAction() == Action.LEFT_CLICK_BLOCK){
                if(event.getClickedBlock().getType() == Material.CRAFTING_TABLE){
                    Location craft = event.getClickedBlock().getLocation();
